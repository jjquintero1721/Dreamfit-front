...
import { PlayCircle, VideoOff, ArrowUpDown } from "lucide-react";
...
export function WorkoutPlanForm({
  ...
  // ── Estado del modal de reorganización ──────────────────────────────────────
  const [reorderModal, setReorderModal] = useState<{ open: boolean; dayIndex: number }>({
    open: false,
    dayIndex: 0,
  });
  ...
  // ── Handler para aplicar el nuevo orden desde el modal ──────────────────────
  const handleApplyReorder = (dayIndex: number, orderedKeys: string[]) => {
    setWorkoutOrderInfo(prev => {
      const updated = { ...prev };
      orderedKeys.forEach((key, index) => {
        if (updated[key]) {
          updated[key] = {
            ...updated[key],
            order: index + 1,
            // Actualizamos timestamp para desempates
    ...
    // Actualizamos el contador del día al máximo nuevo
    setWorkoutOrderCounter(prev => ({
      ...prev,
      [dayIndex]: orderedKeys.length,
    }));
  ...
  // Ejercicios del día activo para el modal
  const orderedExercisesForModal = getOrderedWorkoutsForDay(reorderModal.dayIndex).map(w => ({
    key: w.key,
    name: w.name,
    group: w.group,
  }));
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
                  ...
                  <CardContent className="space-y-6">
                    ...
                    {(selectedGroups[dayIndex] && selectedGroups[dayIndex].length > 0) && (
                      <div className="space-y-4 pt-4 border-t">
                        ...
                        {/* ── Preview del orden actual con botón reorganizar ── */}
                        {getOrderedWorkoutsForDay(dayIndex).length > 0 && (
                          <div className="bg-muted/60 border border-border rounded-xl p-4">
                            <div className="flex items-center justify-between mb-3">
                              ...
                              {/* ── Botón para abrir el modal de reorganización ── */}
                              <Button
                                type="button"
                                variant="outline"
                                size="sm"
                                className="gap-1.5 h-8 text-xs font-medium hover:border-primary/60 hover:text-primary transition-colors"
                                onClick={() =>
                                  setReorderModal({ open: true, dayIndex })
                                }
                              >
      ...
      {/* ── Modal de reorganización de ejercicios ── */}
      <ExerciseReorderModal
        open={reorderModal.open}
        onOpenChange={(open) => setReorderModal(prev => ({ ...prev, open }))}
        exercises={orderedExercisesForModal}
        onSave={(orderedKeys) => handleApplyReorder(reorderModal.dayIndex, orderedKeys)}
      />
    </div>
  );
}
