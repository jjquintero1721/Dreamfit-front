...
import { ArrowLeft, Plus, Dumbbell, ChevronDown, Clock, Play, VideoOff, Edit } from "lucide-react";
...
export default function WorkoutPlanPage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && userRole === 'coach' && userId && id) {
      fetchWorkoutPlan();
    }
  }, [isAuthenticated, id, router, userRole, userId]);

  const fetchWorkoutPlan = async () => {
    setIsLoading(true);
    try {
      const response = await api.get(`/workout-plans/current/${id}`);
  ...
  const getAllWorkoutsForDay = (day: WorkoutPlanResponse["workoutPlan"]["days"][0]) => {
    ...
    day.muscularGroups.forEach(mg => {
      mg.workouts.forEach(workout => {
        allWorkouts.push({
          ...workout,
          muscularGroup: workout.muscularGroup || mg.group,
  ...
  const getFilteredWorkouts = (day: WorkoutPlanResponse["workoutPlan"]["days"][0], selectedMuscleGroup: string | null) => {
  ...
  const getMuscleGroups = (dayWorkouts: WorkoutPlanResponse["workoutPlan"]["days"][0]) => {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Visualiza y gestiona el plan de entrenamiento de tu alumno
        </p>
      </div>
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Creado: {new Date(workoutPlan.created_at).toLocaleDateString()}
                  </p>
                </div>
                <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
                  ...
                  <DialogContent className="max-w-6xl w-[calc(100%-2rem)] mx-auto max-h-[90vh] p-0 overflow-hidden">
            ...
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>Plan de entrenamiento personalizado del alumno</CardDescription>
                </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <h3 className="mt-2 text-lg font-semibold">No hay Plan de Entrenamiento</h3>
              <p className="text-muted-foreground mb-4">
                Este alumno aún no tiene un plan de entrenamiento asignado.
              </p>
              <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
